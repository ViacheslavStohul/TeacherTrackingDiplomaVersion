function AddButton() {
    window.location = `ChangeCommission?id=${-1}`;
}
...
async function DeleteCommission() {
    deleteModal.toggle();
    const query = await fetch(`https://localhost:7113/Commission/DeleteCommission?id=${_Id}`);
    if (query.status !== 200) {
...
function EditButton(id) {
    window.location = `ChangeCommission?id=${id}`;
}
